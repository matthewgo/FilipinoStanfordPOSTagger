 ...
 * computes macro F-measure, accuracy, confidence intervals, Cohen's Kappa
 *
...
public class ConfusionMatrix {
	...
	/**
	 * Micro-averaged F-measure gives equal weight to each document and is
	 * therefore considered as an average over all the document/category pairs.
	 * It tends to be dominated by the classifierâ€™s performance on common
	 * categories. (It's actually the accuracy).
	 * <p/>
	...
	/**
	 * Macro-averaged F-measure gives equal weight to each category, regardless
	 * of its frequency. It is influenced more by the classifier performance on
	 * rare categories.
	 *
	 * @return double
	 */
	 ...
	 *
	 * @param beta
	 *            beta paremeter; higher than 1 prefers recall, lower than 1
	 *            prefers precision
	 * @return double
	 */
	 ...
	 * http://alias-i.com/lingpipe/docs/api/com/aliasi/classify/ConfusionMatrix.
	 * html#confidence95%28%29)
	 *
	 * @return conf
	 */
	...
	public double getCohensKappa() {
		...
		for (String label : this.allGoldLabels) {
			...
			// System.out.println("Label " + label + ", sumCol: " + col + ",
			// sumRow: " + row);
	 ...
	 * uncertainty in semantic tagging. In Proceedings of the 13th Conference of
	 * the European Chapter of the Association for Computational Linguistics,
	 * EACL '12, pages 840-850, Stroudsburg, PA, USA. Association for
	...
	/**
	 * Returns unit matrix (identity matrix) with all diagonal values negative
	 * and non-diagonal zeros
	 * 
	 * <pre>
	 * (-1) * In
	 * </pre>
	...
	/**
	 * Returns a distribution of classes in gold data and predicted data
	...
	/**
	 * Makes probabilistic confusion matrix, where the entries are real numbers
	 * sum up to 1 for each gold row
	 *
	 * @return table
	 */
	...
	public String printDescendingClassDistributionandAccuracy() {
		List<ClassDistribution> cds = getClassDistributionGold();
		StringBuilder sb = new StringBuilder("\tGold \t Count \t Distribution \t Recalled \t Recall %\n");
		Collections.sort(cds, new Comparator<ClassDistribution>() {
			@Override
			public int compare(ClassDistribution cd1, ClassDistribution cd2) {
				return Integer.compare(cd1.getCount(), cd2.getCount()) * -1;
			}
		});
		for (ClassDistribution cd : cds) {
			sb.append(String.format(Locale.ENGLISH, "%12s\t%5d\t%.2f", cd.getLabel(), cd.getCount(),
					(double) cd.getCount() / getTotalSum() * 100.0));
			sb.append("%\t");
			sb.append(String.format(Locale.ENGLISH, "%5d\t%.2f", this.map.get(cd.getLabel()).get(cd.getLabel()),
					getRecallForLabel(cd.getLabel()) * 100));
			sb.append("%");
			sb.append(" " + getClassMistaggedDistribution(cd.getLabel()));
			sb.append("\n");
		}
		return sb.toString();
	}

	public List<ClassDistribution> getClassDistributionGold() {
		List<ClassDistribution> classDistributions = new ArrayList<>();
		for (String goldLabel : this.allGoldLabels) {
			int rowSum = getRowSum(goldLabel);
			classDistributions.add(new ClassDistribution(goldLabel, rowSum));
		}
		return classDistributions;
	}

	public String getClassMistaggedDistribution(String goldValue) {
		List<ClassDistribution> classDistributions = new ArrayList<>();
		Map<String, Integer> predicted = map.get(goldValue);
		for (Map.Entry<String, Integer> entry : predicted.entrySet()) {
			classDistributions.add(new ClassDistribution(entry.getKey(), entry.getValue()));
		}
		Collections.sort(classDistributions, new Comparator<ClassDistribution>() {
			@Override
			public int compare(ClassDistribution cd1, ClassDistribution cd2) {
				return Integer.compare(cd1.getCount(), cd2.getCount()) * -1;
			}
		...
		for (int i = 0; i < classDistributions.size(); i++) {
			ClassDistribution cd = classDistributions.get(i);
			if (cd.getCount() != 0 && !cd.getLabel().equals(goldValue)) {
				sb.append(cd.getLabel() + "-" + cd.getCount() + "(");
				sb.append(String.format("%.2f", cd.getCount() * 100.0 / getRowSum(goldValue)));
				sb.append(")% | ");
			}
