package utils;

public class Constants {
	public static String TRAIN_HPOST_WORDS = "data/HPOST 90.85/trainHPOST.words";
	public static String TRAIN_HPOST_TAGS = "data/HPOST 90.85/trainHPOST.tags";
	public static String TRAIN_HPOST_FEATURES = "data/HPOST 90.85/trainHPOST.features";
	public static String TEST_HPOST_WORDS = "data/HPOST 90.85/testHPOST.words";
	public static String TEST_HPOST_TAGS = "data/HPOST 90.85/testHPOST.tags";
	public static String TEST_HPOST_FEATURES = "data/HPOST 90.85/testHPOST.features";

	public static String TRAIN_STANFORD = "data/stanford/train-filipino";
	public static String TRAIN_STANFORD_FEATURES = "data/stanford/train-filipino-features";
	public static String TEST_STANFORD_TAGGED = "data/stanford/test-filipino-tagged";
	public static String TEST_STANFORD_TAGGED_FEATURES = "data/stanford/test-filipino-tagged-features";
	public static String TEST_STANFORD_TAGGED_SHORT = "data/stanford/test-filipino-tagged-short";
	public static String TEST_STANFORD_UNTAGGED = "data/stanford/test-filipino-untagged";
	public static String TEST_STANFORD_UNTAGGED_SHORT = "data/stanford/test-filipino-untagged-short";
}
